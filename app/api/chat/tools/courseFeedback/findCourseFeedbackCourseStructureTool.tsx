...
const findCourseFeedbackCourseStructureTool = {
  description: `Evaluates the organization and structure of the course, including lectures, problem sessions, and assignments. ${departmentGuardPrompt}. Make sure you don't make up the userCourseId if the user does not provide three or five digit numbers.`,
...
execute: async ({ department, userCourseId, userCourseName, instructor }: { 
    ...
    console.log("firing findCourseFeedbackCourseStructureTool")
    ...
    try {
      ...
      data.forEach((section: any) => {
        ...
        const courseStructure = feedback.courseStructure;

        if (courseStructure) {
          // Collect AI summaries
          allAISummaries.push({
            summary: courseStructure.aiSummary,
          ...
          courseStructure.studentQuotes.forEach((quote: string) => {
            allQuotes.push({
              quote,
              term: section.termOffered || "Unknown Term",
              year: section.year || "Unknown Year",
      ...
      if (allAISummaries.length === 0 && allQuotes.length === 0) {
        return `No course structure feedback found for ${courseName || courseId}${instructor ? ' taught by ' + instructor : ''} in the ${department} department.`;
      }
      ...
      let response = `## Course Structure for ${courseName || courseId}${instructor ? ' taught by ' + instructor : ''}\n\n`;
      ...
      console.error("Error in Course Structure Tool:", error);
      if (error instanceof Error) {
        return `Error fetching course structure: ${error.message}`;
      }
      return "An unexpected error occurred while fetching course structure.";
    }
  }
};

export { findCourseFeedbackCourseStructureTool };