...
const findCourseFeedbackLearningGains = {
  description: `Provides insights into the skills and knowledge students have gained from the course. 
  Includes summaries of learning achievements and relevant student quotes. ${departmentGuardPrompt}. Make sure you don't make up the userCourseId if the user does not provide three or five digit numbers.`,
...
execute: async ({ department, userCourseId, userCourseName, instructor }: { 
    ...
    console.log("firing findCourseFeedbackLearningGains")
    ...
    try {
      ...
      data.forEach((section: any) => {
        ...
        const learningGains = feedback.learningGains;

        if (learningGains) {
          // Collect AI summaries
          allAISummaries.push({
            summary: learningGains.aiSummary,
          ...
          learningGains.studentQuotes.forEach((quote: string) => {
            allQuotes.push({
              quote,
              term: section.termOffered || "Unknown Term",
              year: section.year || "Unknown Year",
      ...
      if (allAISummaries.length === 0) {
        return `No learning gains feedback found for ${courseName || courseId}${instructor ? ' taught by ' + instructor : ''} in the ${department} department.`;
      }
      ...
      let response = `## Learning Gains for ${courseName || courseId}${instructor ? ' taught by ' + instructor : ''}\n\n`;
      ...
      console.error("Error in Learning Gains Tool:", error);
      if (error instanceof Error) {
        return `Error fetching learning gains: ${error.message}`;
      }
      return "An unexpected error occurred while fetching learning gains.";
    }
  }
};

export { findCourseFeedbackLearningGains };